...
public class Conv extends RelTerm {
    private final RelTerm body;

    public Conv(final RelTerm body) {
        this.body = body;
    }

    @Override
    public String toStringPrec(int precedence) {
        return this.body.toStringPrec(2) + '^';
    }
    ...
    protected void computeGraph(Object parent, mxCell source, mxCell target, boolean forward, mxGraph graph, int x,
        ...
        if (interPoints.isEmpty()) {
            graph.insertEdge(parent, null, body, from, to);
        } else {
            String fctName = body.toStringPrec(y) + System.nanoTime(); // create a unique name for the function.
            ...
            graph.insertEdge(parent, null, body, from, to, "defaultEdge;edgeStyle=" + fctName);
        }
            }
}
